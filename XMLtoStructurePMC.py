...
class PubmedEntry:
    ...
    def to_sentences(self):
        ...
        abstractSents = self._makeSentences(abstracts)

        sections = [self.sections[x] for x in self.sections]
        textSents = self._makeSentences(sections)

        floatSects = [self.floats[x] for x in self.floats]
        floatSents = self._makeSentences(floatSects)
        ...
        if len(floatSents) > 0:
            floatSents = self._prepareSentences(str(self.pmid), 4, floatSents)
    ...
    @classmethod
    def get_floattext(cls, node):

        # TODO care for structued abstracts https://www.nlm.nih.gov/bsd/licensee/elements_descriptions.html#publicationtypelist
        structuredText = {}

        abstractTexts = cls.get_nodes(node, './/floats-group')

        for secText in abstractTexts:

            secTitle = cls.get_inner_text_from_path(secText, "label")
    ...
    def fromXMLNode(cls, node):
        ...
        pmidIDNode = cls.get_node_with_attrib(node, 'front/article-meta/article-id', 'pub-id-type', 'doi')
        pmcIDNode = cls.get_node_with_attrib(node, 'front/article-meta/article-id', 'pub-id-type', 'manuscript')
        ...
        floatText = cls.get_floattext(node)
...
if __name__ == '__main__':
    ...
    storagePath = '/mnt/biocluster/projekte/Corona2020/Texts/2021_07_01/EUPMCfulltexts/'
    ...
    def senteniceFile(filenames, env):
        ...
        for filename in filenames:
            ...
            with open(storagePath + sentfile, 'w') as outfile, open(storagePath + datefile, 'w') as outdate, open(storagePath + typefile, "w") as outtype, open(storagePath + pmidfile, "w") as outpmid:
                ...
                for elem in pubmedParser.tree.getroot().getchildren():
